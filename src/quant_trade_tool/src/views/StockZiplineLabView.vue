...
import {
  stocksApi,
  type StockZiplineBacktestResult,
  type StockZiplineComboLeg,
  type StockZiplineRunSummary,
  type StockZiplineStrategy,
  type StockZiplineTimeframeOption,
} from "@/api/stocks";
...
const timeframeOptions = computed(() =>
  status.value?.timeframes?.length
    ...
    : [{ id: "1d", label: "日线", bar_minutes: 1440 }],
);
...
const strategies = ref<StockZiplineStrategy[]>([]);
const runs = ref<StockZiplineRunSummary[]>([]);
const result = ref<StockZiplineBacktestResult | null>(null);

const symbolsText = ref("600519,000001");
const form = ref({
  symbol: "600519",
  timeframe: "1d",
...
async function loadStatus() {
  try {
    const { data } = await stocksApi.ziplineStatus(dataDir, undefined, form.value.timeframe);
    status.value = data;
...
function buildComboLegs(): StockZiplineComboLeg[] | undefined {
...
async function downloadData(format: "csv" | "zip") {
  ...
  error.value = "";
  try {
    await stocksApi.ziplineExportDownload({
...
async function setupZiplineVenv() {
  ...
  error.value = "";
  try {
    const { data } = await stocksApi.ziplineSetupVenv();
    if (data.ok) {
...
async function loadStrategies() {
  const { data } = await stocksApi.ziplineStrategies();
...
async function loadRuns() {
  try {
    const { data } = await stocksApi.ziplineRuns(dataDir, 15);
...
async function syncData() {
  ...
  try {
    const { data } = await stocksApi.ziplineSync({
      symbols,
      data_dir: dataDir,
      backfill_days: 800,
    });
...
async function runBacktest() {
  ...
  result.value = null;
  try {
    const { data } = await stocksApi.ziplineBacktest({
      symbol: form.value.symbol,
...
async function openRun(runId: string) {
  loading.value = true;
  try {
    const { data } = await stocksApi.ziplineRun(runId, dataDir);
    result.value = data;
  } catch (e) {
...
<template>
  <div v-loading="loading">
    <h1 class="page-title">A股 策略实验室</h1>
    <p class="page-desc">
      A 股日线回测（1d），支持多策略组合；优先 <strong>zipline-reloaded</strong>（<code>.venv-zipline</code>）。
    ...
    <el-card shadow="never" class="panel-card">
      ...
      <el-form inline size="small">
        ...
        <el-form-item label="标的">
          <el-input v-model="symbolsText" placeholder="600519,000001" style="width: 160px" />
        </el-form-item>
        <el-form-item>
          <el-button type="primary" :loading="syncing" @click="syncData">同步日线</el-button>
        </el-form-item>
        <el-form-item>
    ...
    <el-card shadow="never" class="panel-card mt">
      ...
      <el-table v-else :data="runs" size="small" @row-click="(row: StockZiplineRunSummary) => openRun(row.run_id)">
