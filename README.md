...
- **A股 策略实验室**（`/stock-zipline`）— 日线回测（`data/stocks/{SH600519}/ohlcv.csv` → XSHG 日历 csvdir bundle）、复用同一套 19 个策略与组合模式；`POST /api/v1/stocks/zipline/sync` 拉取日线；`GET /api/v1/stocks/zipline/data/export?format=csv|zip`
