"""Backtest runner: zipline-reloaded via isolated .venv-zipline subprocess."""
...
def run_zipline_subprocess(
    *,
    ...
    proc = subprocess.run(
        [str(py), "-m", "quant_rd_tool.crypto_zipline_worker"],
    ...
    if proc.returncode != 0:
        raise RuntimeError(
            f"Zipline worker failed (code {proc.returncode}):\n{proc.stderr or proc.stdout}"
        )
...
def run_zipline_backtest(
    *,
    ...
    raise RuntimeError(
        "Zipline venv not ready. Run POST /api/v1/crypto/zipline/setup-venv or use engine=pandas."
    )
...
def run_backtest(
    *,
    ...
    if engine == "zipline" and not z_ok:
        raise RuntimeError(
            f"zipline-reloaded not available. Use POST /crypto/zipline/setup-venv. Error: {z_err}"
        )
    ...
    if use_zipline:
        try:
            ...
            logger.warning("Zipline backtest failed, falling back to pandas: %s", exc)
            if engine == "zipline":
                raise
